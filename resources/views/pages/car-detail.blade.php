...
@section('title', $car->title . ' - ThriftMotors')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            ...
            <div x-data="{ 
                    activeImage: 0, 
                    images: [{!! $car->images->map(fn($img) => "'".$img->image_url."'")->join(',') !!}],
                    ...
                    <div class="grid grid-cols-6 gap-2">
                        @foreach($car->images as $index => $image)
                            <button @click="activeImage = {{ $index }}"
                                    :class="{ 'ring-2 ring-red-600': activeImage === {{ $index }} }"
                                    class="rounded-lg overflow-hidden border-2 border-gray-300 dark:border-gray-600">
                                <img src="{{ $image->image_url }}" alt="Thumbnail" class="w-full h-16 object-cover">
                            </button>
                        @endforeach
                    </div>
                @endif
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-transparent dark:border-gray-700 p-6 mb-6 transition-colors duration-300">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h1 class="text-3xl font-bold mb-2 dark:text-white">{{ $car->title }}</h1>
                        <p class="text-gray-600 dark:text-gray-400">
                            <i class="fas fa-clock mr-2"></i>Listed {{ $car->created_at->diffForHumans() }}
                        </p>
                    </div>
                    @if($car->statusBadge)
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                    <div><p class="text-gray-600 dark:text-gray-400 text-sm">Year</p><p class="text-xl font-semibold dark:text-gray-200">{{ $car->year }}</p></div>
                    <div><p class="text-gray-600 dark:text-gray-400 text-sm">Mileage</p><p class="text-xl font-semibold dark:text-gray-200">{{ number_format($car->mileage) }} km</p></div>
                    <div><p class="text-gray-600 dark:text-gray-400 text-sm">Color</p><p class="text-xl font-semibold dark:text-gray-200">{{ $car->color }}</p></div>
                    <div><p class="text-gray-600 dark:text-gray-400 text-sm">Seats</p><p class="text-xl font-semibold dark:text-gray-200">{{ $car->seats }}</p></div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-transparent dark:border-gray-700 p-6 mb-6 transition-colors duration-300">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div class="border-l-4 border-red-600 pl-4 py-2">
                        <p class="text-gray-600 dark:text-gray-400 text-sm">Brand</p><p class="font-semibold dark:text-gray-200">{{ $car->brand->name }}</p>
                    </div>
                    <div class="border-l-4 border-red-600 pl-4 py-2">
                        <p class="text-gray-600 dark:text-gray-400 text-sm">Fuel Type</p><p class="font-semibold dark:text-gray-200">{{ $car->fuelType->name }}</p>
                    </div>
                    <div class="border-l-4 border-red-600 pl-4 py-2">
                        <p class="text-gray-600 dark:text-gray-400 text-sm">Transmission</p><p class="font-semibold dark:text-gray-200">{{ $car->transmission->name }}</p>
                    </div>
                </div>
            </div>
        ...
        <aside class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-transparent dark:border-gray-700 p-6 mb-6 sticky top-20 transition-colors duration-300">
                ...
                <p class="text-4xl font-bold text-red-600 dark:text-red-500 mb-1">KES {{ number_format($car->price, 0) }}</p>
                ...
                <div class="space-y-3 mb-4">
                    ...
                    {{-- WhatsApp --}}
                    <a href="https://wa.me/{{ preg_replace('/\D/', '', $car->seller_whatsapp ?? $car->seller_phone) }}?text={{ urlencode('Hi, I am interested in the '.$car->title.' on ThriftMotors. Price: KES '.$car->price) }}"
                       target="_blank"
                ...
                <div class="border-t dark:border-gray-700 pt-4 mt-6">
                    ...
                    <a href="tel:{{ $car->seller_phone }}" class="text-red-600 dark:text-red-400 hover:underline text-sm">{{ $car->seller_phone }}</a>
                </div>
            </div>
        </aside>
    </div>
...
{{-- ===================================================
     RESERVATION MODAL (Alpine.js)
     =================================================== --}}
<div
    x-data="reservationModal({{ $car->id }}, '{{ addslashes($car->title) }}')"
    x-show="open"
    x-cloak
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md z-10 border border-transparent dark:border-gray-700 transition-colors duration-300" @click.stop>
        ...
        <div class="flex items-center justify-between p-6 border-b dark:border-gray-700">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $car->title }}</p>
            </div>
            ...
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-xl p-4">
                <div class="flex items-center gap-3">
                    <div class="text-3xl font-bold text-red-600 dark:text-red-500">
                        KES {{ number_format(config('thriftmotors.deposit_amount', 5000)) }}
                    </div>
...
<div id="inquiryModal" class="hidden fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 transition-opacity">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full border border-transparent dark:border-gray-700">
        ...
        <form action="{{ route('inquiries.store', $car) }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div>
